...
that answers it. All verified on a live Odoo 18 instance.
...
|---|---|---|
| `preinvoice_base` | yes | **0** invoices flagged `is_preinvoice` |
| `fb_odoo_lead` | yes | 31 UTM sources exist, but **all 84 leads have `source_id = False`** |
| `mailing` | yes | 812 contacts, **1** campaign |
| `purchase` | yes | 3 orders against 653 vendor bills |

```python
# capability exists?
"is_preinvoice" in odoo.fields_get("account.move", [], ["type"])
# capability used?
odoo.search_count("account.move", [["is_preinvoice", "=", True]], CTX)
```
...
```
--url https://app.persevida.com --key <KEY>
  database   persevida
  login      bo@fl1.cz  (resolved from the API key, not supplied)
```
...
Two leads against 89 opportunities means the "Leads" setting is off and
everything enters as an opportunity — say that, do not report "2 leads" as a
business fact.
...
Read `crm_leads.md` before converting.
...
Three levels, measured on 107 payments:

```
state = paid            what the record thinks
move_id set             83 — an accounting entry exists
is_matched = True       11 — the BANK confirmed it
```

Answering from `state` alone overstates collected cash by 7×. Full detail
...
|---|---:|---:|
| tickets | 53 | **6** |
| leads | 84 | **13** (71 archived) |
| projects | 87 | depends on stage |
...
Measured on the reference instance — 364 invoices in EUR, 8 in CZK:

```
amount_total          14.314.447,65     <- meaningless: EUR + CZK added up
amount_total_signed    1.197.262,64     <- the real figure

Top customer, same data:
  amount_total          13.552.000      <- 12x overstated
  amount_total_signed      550.331      <- correct
```

Eight invoices out of 372 were enough to inflate the total twelvefold. A
...
The gap is large and easy to get wrong: on the reference instance there are
**53 tickets in total but only 6 open** — the rest are Solved or Cancelled.
Answering "53 tickets in the queue" to "how many open tickets?" overstates
the backlog ninefold.
...
**Trap:** `crm.lead` hides archived records by default. On the reference
instance 84 exist but a plain query returns 13 — the other 71 are archived.
Say which you mean:
