...
c = Collab(connect(), company_id=2)
```

Everything below was verified on a live instance — 11 operations, all green.
...
alternative subtype `mt_comment` emails every follower, and on this instance
12 of 12 sampled orders had a customer among their followers. When in doubt,
...
a = c.todo("sale.order", sid, "Call the customer back",
           ...
           type_name="call")
# {'id': 355, 'summary': 'Call the customer back',
#  'user_id': [7, 'Daniel De Vecchi'], 'date_deadline': '2026-08-14',
#  'state': 'planned'}

c.postpone(a["id"], days=10)
# {'from': '2026-08-14', 'to': '2026-08-21', 'changed': True}

c.reassign(a["id"], 2)
# {'from': [7, 'Daniel De Vecchi'], 'to': [2, 'Roberto Crotti'], 'changed': True}

c.done(a["id"], "Customer contacted, 10% discount confirmed.")
# {'done': True, 'logged_in_chatter': True, 'record': 'sale.order/766'}
...
— that message is the only surviving record of the work. Verified: chatter
went 3 → 4 messages. Always pass real feedback; an empty one leaves a note
saying nothing.
...
odoo.call("mail.activity", "write", [[aid]],
          {"date_deadline": "2026-08-21", "context": CTX})

# RIGHT
odoo.call("mail.activity", "write", [[aid], {"date_deadline": "2026-08-21"}],
          {"context": CTX})
```
...
This instance has 17, several bound to a model:

```
Email +0d · Call +2d · Call +1d · Meeting +0d · To-Do +5d
Order Upsell (sale.order) · Tax Report Ready (account.move)
Financial Statement (res.partner, +12 months)
```
...
```python
c.pending(user_id=2)
# {'total': 4, 'overdue': 0, 'today': 0, 'items': [...]}

c.pending(overdue_only=True)      # what is late
c.workload()
# [{'user': 'Roberto Crotti', 'count': 4}, {'user': 'OdooBot', 'count': 1}]
```
...
```
calendar.event        1175    the meeting
calendar.attendee     2057    one row per invited partner, holds the RSVP
calendar.recurrence      5    the rule behind a repeating series
calendar.alarm          59    reminders (notification or email)
```
...
```python
ev = c.meet("Quarterly review", start, users=[2, 7], minutes=90,
...
```
Daniel De Vecchi     needsAction
Roberto Crotti       accepted
```
...
```
create(..., res_model="sale.order", res_id=766)
  -> res_model: False, res_model_id: False, res_id: 766
```
...
```
write(res_model_id=576)
  -> res_model: 'sale.order', res_model_id: [576, 'Sales Order'], res_id: 766
```

`c.meet(link_to=(model, id))` resolves this for you. On this instance 81 of
1175 events are linked — 71 to `project.task`, 8 to `crm.lead`.

## RSVPs

```python
c.invite_status(ev["id"])
# {'total': 2, 'accepted': ['Roberto Crotti'], 'declined': [],
#  'tentative': [], 'no_answer': ['Daniel De Vecchi']}
...
Across the whole instance: 1189 accepted, 854 never answered, 7 declined,
7 tentative. **Roughly 40% of invitations are never answered** — do not read
`needsAction` as a refusal.
...
```
recurrence 7  "Every 1 weeks on Wednesday until 2024-12-23"
rrule: DTSTART:20240904T000000
       RRULE:FREQ=WEEKLY;UNTIL=20241223T235959;BYDAY=WE
  -> 16 calendar.event rows
```
...
meetings counts occurrences, not series: 43 of the 1175 events here belong
to just 5 recurrences.

## Reminders

```
Notification - 15 Minutes / 30 Minutes / 1 Hours / 2 Hours / 1 Days
Email - 3 Hours
```
...
4. **Check `invite_status()`** before assuming a meeting is confirmed. 40% of
   invitations on this instance were never answered.
