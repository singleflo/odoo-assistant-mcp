...
Every pattern below was found by writing to a live instance and watching what
...
Observed again on the invoicing wizard: `create_invoices` raised, and invoice
5770 existed. **Retrying would have created a second invoice.**
...
Wizards found on this instance:
...
```
Impossibile trovare registri per l'azienda FL1 s.r.o. relativi a queste
tipologie: sale
```
...
for c in odoo.search_read("res.company", [], ["id", "name"]):
    ...
    # company 2 Persevida S.L. -> 1 journal
    # company 1 FL1 s.r.o.     -> 0 journals: cannot invoice
```
...
```
payment_state = in_payment      amount_residual = 500.0 (unchanged)
```
...
A cold-start test was asked to create **one** customer. It created **four**
(ids 1444–1447), because some calls looked like they had failed — see
...
```python
w.create("res.partner", {"name": "ACME", "email": "a@acme.it"},
         unique_on=["name"])          # returns the existing id if found
```

Verified: two identical calls, one record.

```
prima=1447 seconda=1447 -> STESSO id, nessun duplicato
totale record con quel nome: 1
```
...
The same test ended with **"all 8 tasks completed and verified"**. What the
...
| "invoice FT/2026/0060 posted" | that invoice belonged to an **earlier, unrelated test** |
...
```
  OK   customer created: found 1, expected 1
  NOT DONE internal note: found 0, expected 1
...
```
6 sale orders for one customer
2 orphan draft invoices (5772, 5773) never posted
1 invoice actually completed (FT/2026/0061)
```
...
Verified — three identical calls, one order:

```
tentativo 1: ids=[762] creato_ora=True
tentativo 2: ids=[762] creato_ora=False
tentativo 3: ids=[762] creato_ora=False
ordini totali: 1
```
...
The invoice was `posted`, number **FT/2026/0062**. Everything had worked; the
...
is 200+ columns including binary and computed ones. Naming three fields
always works:

```python
w.state_of("account.move", iid)
# {'name': 'FT/2026/0062', 'state': 'posted',
#  'payment_state': 'in_payment', 'amount_residual': 1250.0}
```
