...
Merged from older Odoo skills. **Verify claims against the target Odoo 18
instance** before relying on them; version-specific fields can differ.
...
Verify each field against `fields_get` on the target Odoo 18 instance:
...
The common states and methods should be checked on the target instance. Methods: `action_validate()`,
...
This is where money questions go wrong. Measure these categories on the target
instance:

```
move_id + is_matched : <example count>    posted AND reconciled with the bank
move_id, not matched : <example count>    posted, bank has not confirmed
no move_id           : <example count>    NO accounting entry exists at all
```
...
payment as received on `state = paid` alone can materially overstate cash.
...
```
<electronic method>    move_id=NO   <example count>
<electronic method>    move_id=NO   <example count>
<electronic method>    move_id=SI  <example count>      <- electronic WITH an entry
<manual method>        move_id=NO  <example count>      <- manual WITHOUT one
<manual method>        move_id=SI  <example count>
```

**Missing entries are not a property of the payment method.** Some manual
payments lack `move_id` too, and some electronic payments have one. The pattern
...
`default_account_id` (final liquidity). Check every line; the trap may or may
not be present on the target instance.
...
Inspect the transaction states on the target instance:

```
done <example count> · pending <example count> · error <example count> · draft <example count>
```

A transaction in `done` does not imply the payment is reconciled: one
transaction can be `done` while its related transaction is `error`. Read the payment,
...
Passing an unauthorized company id raises it immediately.
...
Check whether `l10n_it_edi_state` is used before treating the failure below
as an observed instance condition. The `l10n_it_*` fields may be installed but
unused.
...
Review any invoices with a `ref` exceeding 20 characters (for example,
`<example reference>`); if Italian EDI is switched on, those records would
need review.
...
- **The old API scripts** (`call_method.py`, `search_read.py`, …) — transport-
  specific, exited instead of raising exceptions, and used unsafe defaults.
  `odoo_client.py` supersedes them with auto-detected transport.
- **Instance profiles** (`<instance-profile>.md`) — instance-specific
